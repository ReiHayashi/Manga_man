...
if(!empty($_GET['id'])){
              ...
              <div class="card-body">
                <form method="POST">
                  ...
                  <div class="wrapper my-3">
                    <input class="btn btn-success btn-block" type="submit" name="submit1" value="Save Changes" required>
                  </div>
                </form>
              </div>
            </div>
 ...
 <?php
  ...
  if($_SESSION['username'] === $row['U']){
  $username = $_SESSION['username'];
  $reviewthing = 'SELECT V.*,
  review.username as U, review.datesubmitted as DS, series.primaryname as PN, review.id as reviewid, review.content as C
  FROM volumes AS V
  INNER JOIN reviews_in_volumes AS RIS ON RIS.volume_id = V.id
  INNER JOIN review ON RIS.review_id = review.id
  ...
  INNER JOIN series ON VIS.series_id = series.series_id
  WHERE review.id='.$id;
  $reviewquery = mysqli_query($connection, $reviewthing);
  $row1 = mysqli_fetch_array($reviewquery);
  ?>
  <section id="details_header" class="py-4 mb-4 bg-dark">
    <div class="container">
      <div class="row">
    ...
    <div class="container">
      <div class="row">
        <div class="col my-2">
          <div class="card bg-dark">
            <div class="card-header">
              <h4>Edit review for: <?php echo $row1['PN'].' '.$row1['title'].'<br> Review by: '.$row1['U']; ?></h4>
            </div>
            <div class="card-body">
              <form method="POST">
                <div class="form-group">
                  <label class="font-weight-bold" for="Synopsis">Content:</label>
                  <textarea class="form-control" name="content" ><?php echo $row1['C']; ?></textarea>
                </div>
                <div class="wrapper my-3">
